<div>
    {{-- <div class="flex flex-wrap justify-center items-center gap-6">
        @for ($i = 0; $i <= 12; $i++)
            <div class="shadow-lg">
                <img src="{{ $produk[1]['foto'] }}" alt="foto produk" class="w-72 rounded-xl">
            </div>
        @endfor
    </div> --}}
    ...
    <div class="flex flex-wrap justify-center items-center gap-6">
        @foreach ($produk as $item)
            <div class="shadow-lg">
                <img src="{{ $item->foto }}" alt="foto produk" class="w-72 rounded-xl">
            </div>
        @endforeach
    </div>
    ...
    @if ($count < $total)
        <div class="mt-14 text-center">
            <button class="btn bg-redMain border-redMain text-white" wire:click='showMore'>
                Show more
                <span wire:loading class="text-white ml-2 animate-spin-slow">
                    <svg xmlns="http://www.w3.org/2000/svg" class="fill-current w-10 h-10" viewBox="0 0 24 24">
                        <path
